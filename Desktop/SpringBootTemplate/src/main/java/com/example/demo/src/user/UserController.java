...
public class UserController {
    ...
    public BaseResponse<PostUserRes> createUser(@RequestBody PostUserReq postUserReq) throws BaseException {
        ...
        // 가입 시 닉네임과 password에 관한 논리적 Validation 추가
        ...
        // 닉네임과 password 길이 수 제한 등 형식적 Validation 추가
        if(length(postUserReq.getNickname()) > 10){
            return new BaseResponse<>(POST_USERS_INVALID_NICKNAME);
        }
        if(length(postUserReq.getPassword()) < 5){
        ...
        if (this.userProvider.checkEmail(email) == 1) {
            return new BaseResponse<>(POST_USERS_REGISTERED_EMAIL);
        }
    ...
    public BaseResponse<PostLoginRes> logIn(@RequestBody PostLoginReq postLoginReq) {
        try {
            ...
            if(length(postLoginReq.getPassword()) < 5 ){
                return new BaseResponse<>(POST_USERS_INVALID_PASSWORD);
            }
    ...
    @GetMapping("{NickName}") // (GET) 127.0.0.1:9000/app/users
...
//            //같다면 유저네임 변경
    ...
    @ResponseBody
    @GetMapping("{email}")
    public BaseResponse<List<GetUserRes>> getuser (@PathVariable("email") String email){
        try{
            GetUserRes getUserRes = userProvider.getUser(email);
            return new BaseResponse<List>(getUserRes);
        } catch(BaseException exception){
            return new BaseResponse<>((exception.getStatus()));
        }
    }
    ...
    public BaseResponse<String> leftUserEmail(@PathVariable("userIdx") int userIdx, @RequestBody User user) {
        try {
            ...
            GetUserRes getUserRes = new GetUserRes(userIdx, user.getNickname(),user.getEmail(),user.getPassword(),user.getLeft());
            userService.leftUserEmail(getUserRes);
