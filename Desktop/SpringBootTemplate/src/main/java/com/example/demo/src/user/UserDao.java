...
public class UserDao {
    ...
    //회원탈퇴 처리
    public int leftUser(GetUserRes getUserRes){
        String leftuserQuery = "update User set left = 1 where email = ?";
        Object[] leftuserParams = new Object[]{getUserRes.getEmail()};
        return this.jdbcTemplate.update(leftuserQuery,leftuserParams);
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, password,email,nickname,left from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public List<GetUserRes> getUsers() {
        ...
        return this.jdbcTemplate.query(getUsersQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("Email"),
                        rs.getString("password"),
                        rs.getInt("left")) // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
    ...
    public GetUserRes getUserbyEmail(String email){
        String getUserbyEmailQuery = "select * from user where userIdx =?";
    ...
    //탈퇴 회원인지 확인
    public int checkLeft(int userIdx){
        String checkUserQuery = "select left from user where useridx = ?";
        int getUserParams = userIdx;
        return this.jdbcTemplate.queryForObject(checkUserQuery,int.class,getUserParams);
